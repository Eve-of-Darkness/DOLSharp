...
namespace DOL.GS.Quests
{
    ...
    public class DataQuest : AbstractQuest
    {
                ...
                if (!string.IsNullOrEmpty(lastParse))
                {
                    ...
                    foreach (string str in parse1)
                    {
                        if (str == string.Empty)
                        {
                            ...
                            m_targetNames.Add(string.Empty);
        ...
        protected virtual bool ExecuteCustomQuestStep(GamePlayer player, int step, eStepCheckType stepCheckType)
        {
            ...
            if (!string.IsNullOrEmpty(m_classType))
            {
                ...
                if (m_customQuestStep == null)
                {
                    ...
                    if (QuestPlayer != null)
                    {
                        ChatUtil.SendDebugMessage(QuestPlayer, string.Format("Failed to construct custom DataQuest step of ClassType {0}!  Quest will continue anyway.", m_classType));
                    }
                }
            }
        ...
        {
            if (m_charQuest == null || m_charQuest.IsPersisted == false)
            {
                return;
            }
